		...
		IDriverCallback& rCallback)
{
	...
	if(l_pSignalFormat!=NULL)
	{
		...
		{
			m_vExponentChannel.push_back(l_pSignalFormat[i].UnitExponent+6/*changed measure unit in uV*/);
		...
		for(uint32 j = 0; j < m_vHandleSlaves.size() ; j++)
		{
			...
			if(l_pSignalFormat!=NULL)
			{
				m_rDriverContext.getLogManager() << LogLevel_Trace << ">Slave device ndeg" << (uint32)j <<
